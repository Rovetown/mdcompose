...
- **A shared, browsable community snippet library, gated behind the TUI.** Concept borrowed from the skill marketplaces appearing around Claude Code Skills: a public GitHub repository as the source, crawled and indexed through GitHub's API into a locally cached, fuzzy-searchable list of community-contributed snippets, browsed and pulled into your own library one at a time from inside the TUI.
  ...
  Security is scoped just as narrowly: the source is one fixed GitHub API host and a named repository, never an arbitrary URL, so this cannot become a general-purpose fetcher; indexing pins a specific commit rather than a floating branch head, so what you browse is stable and namable in a bug report; every crawled file is subject to the same size limit as any other file mdcompose reads; and crawled content is markdown only, composed like any snippet, never executed.
